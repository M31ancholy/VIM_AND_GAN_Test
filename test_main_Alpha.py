...
def extract_features(image_tensor, model): # 提取特征
    with torch.no_grad():
        features = model.forward(image_tensor, return_features=True, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False)
    return features
...
def main():
    ...
    for epoch in range(20000):
        ...
        for idx, (slice_image, file_name) in enumerate(dataloader):
            ...
            print("DataLoader提供的图片的shape:{}".format(slice_image.shape))
            ...
            print("从VIM提取出来的特征向量形状是：{}".format(train_features.shape))
            ...
            # if idx % 100 == 0:
            #     # 保存生成的图像
            #     input_tensor = gen_imgs.squeeze(0)
            #     to_pil = ToPILImage()
            #     image = to_pil(input_tensor.cpu())
            #     image.save(f'output_images/output_image_{epoch}_{idx}.png')
    writer.close()
