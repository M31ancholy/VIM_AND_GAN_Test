...
class NiiDataset(Dataset):
    def __init__(self, nii_dir, transform=None):
        ...
        self.data = []  # 存储处理后的图像数据和文件名
        self.file_names = []  # 存储文件名

        # 遍历目录中的所有文件，加载图像数据并存储
        for file_name in os.listdir(nii_dir):
            if file_name.endswith('.nii') or file_name.endswith('.nii.gz'):
                ...
                img_data = nii_image.get_fdata()  # 获取图像数据并缓存

                # 处理每个切片
                for i in range(img_data.shape[2]):  # 假设图像是 3D 的
                    slice_data = img_data[:, :, i]  # 获取特定切片
                    ...
                    self.data.append(slice_image)  # 将处理后的图像添加到 data 列表中
                    self.file_names.append(file_name)  # 将文件名添加到 file_names 列表中
    ...
    def __getitem__(self, idx):
        ...
        - 处理后的 PIL 图像和文件名。
        """
        return self.data[idx], self.file_names[idx]  # 返回处理后的图像和文件名
class NiiDataset1(Dataset):
    def __init__(self, nii_dir, transform=None):
        """
        初始化 NiiDataset 类。
...
def train_gan(generator, discriminator, train_features, real_imgs, optimizer_G, optimizer_D, the_loss, device):
    ...
    if step % 300 == 0:
        ...
        # 计算生成器的损失
    g_adv_loss = the_loss(discriminator(gen_imgs), valid)  # 对抗损失
    criterion_pixelwise = torch.nn.MSELoss() #MSE_loss
    g_pix_loss = criterion_pixelwise(gen_imgs, real_imgs)  # 像素级损失
    g_loss = g_adv_loss + 200 * g_pix_loss  # 结合对抗损失和像素级损失
    ...
    real_loss = the_loss(real_pred, valid)
    fake_loss = the_loss(fake_pred, fake)
    ...
    # # 训练判别器
    # optimizer_D.zero_grad()
    # real_loss = criterion_pixelwise(gen_imgs,real_imgs)
...
def main():
    ...
    gen_path = '/DCGAN_STATE/generator_state_epoch_9.pth'
    if os.path.exists(gen_path):
        generator.load_state_dict(torch.load(gen_path))
    else:
        print(f"Warning: {gen_path} does not exist. Starting with a new generator.")


    discriminator = dcgan_modified.Discriminator().to(device)
    disc_path = "/DCGAN/discriminator_state_epoch_9.pth"
    if os.path.exists(disc_path):
        discriminator.load_state_dict(torch.load(disc_path))
    else:
        print(f"Warning: {disc_path} does not exist. Starting with a new discriminator.")
    ...
    # 加载 Nii 数据
    nii_dir = "CC-359-Original"
    print("Start loading nii_files")
    ...
    for epoch in range(10,total_epoch):
        ...
        for idx, (slice_image, file_name) in enumerate(dataloader):
            ...
            # print(train_features.shape) #[1,192]
            ...
            print(f"Epoch [{epoch}/{total_epoch}] Batch [{idx}/{len(dataloader)}] - G Loss: {g_loss:.4f}, D Loss: {d_loss:.4f}")

        torch.save(generator.state_dict(), 'DCGAN_STATE/generator_state_epoch_{}.pth'.format(epoch))
        torch.save(discriminator.state_dict(), 'DCGAN_STATE/discriminator_state_epoch_{}.pth'.format(epoch))
    writer.close()
